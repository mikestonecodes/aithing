...
draw_pipes :: proc(app: ^App, top: f32, view: Rect) {
	...
	for i in 0 ..< len(app.snake) - 1 {
		...
		// Grey, like the stones it runs between, except the last length into
		// the stone the path ends on, which wears that stone's colour. It was
		// the two stones' colours mixed, which strung a band of tints along a
		// path whose stones had all gone grey: the colour on screen is for
		// where the thread has got to, and the wire leads the eye into it.
		col := color_alpha(MUTED, 0.35)
		if i + 1 == len(app.snake) - 1 do col = color_alpha(b.col, 0.6)
		if abs(ay - by) < 1 {
			back := b.r.x < a.r.x
...
draw_tile :: proc(app: ^App, t: Tile, r: Rect, open: bool, last: bool) {
	...
	// path ends on wears its colour — that, and the one under the pointer,
	// which comes up in full on the same spring it lifts on. Each used to be
	// tinted with its own family's, fill and mark both, which is twenty
	// coloured squares in a row with nothing to say which of them mattered;
	// the shape of the mark says the family, and the colour is left to say
	// where the thread has got to and which stone is being looked at.
	lit := last ? 1 : clamp(pop, 0, 1)
	base := color_mix(PANEL, t.col, (0.2 + 0.3 * pop) * lit + 0.12 * live)
	if t.answer do base = color_mix(PANEL, GOLD, (0.3 + 0.25 * pop) * lit)
	if t.user do base = color_mix(PANEL, color_mix(USER_BG, ACCENT, 0.14 + 0.24 * pop), lit)
